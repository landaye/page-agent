	...
	return (
		<div className="flex flex-col h-screen bg-background">
			...
			<header className="flex items-center gap-2 border-b px-3 py-2">
				...
				<span className="text-sm font-medium flex-1">{i18n.t('ui.extension.history.title')}</span>
				{sessions.length > 0 && (
					<Button
						variant="ghost"
						size="sm"
					...
					>
						...
						{i18n.t('ui.extension.history.clearAll')}
					</Button>
				)}
			</header>
			...
			<div className="flex-1 overflow-y-auto">
				{loading && (
					<div className="flex items-center justify-center h-32 text-xs text-muted-foreground">
						{i18n.t('ui.extension.history.loading')}
					</div>
				)}

				{!loading && sessions.length === 0 && (
					<div className="flex items-center justify-center h-32 text-xs text-muted-foreground">
						{i18n.t('ui.extension.history.noHistory')}
					</div>
				)}
						...
						<div className="flex-1 min-w-0">
							...
							<div className="flex items-center mt-0.5">
								<p className="text-[10px] text-muted-foreground">
									...
									{i18n.t('ui.extension.history.steps')}
								</p>
								<div className="flex items-center gap-0.5 ml-auto opacity-0 group-hover:opacity-100 transition-opacity">
									<button
										...
										title={i18n.t('ui.extension.history.runAgain')}
										aria-label={`${i18n.t('ui.extension.history.runAgain')}: ${session.task}`}
									>
									...
									<button
										...
										title={i18n.t('ui.extension.history.exportJson')}
										aria-label={`${i18n.t('ui.extension.history.exportJson')} for ${session.task}`}
									>
									...
									<button
										...
										title={i18n.t('ui.extension.history.deleteHistory')}
										aria-label={`${i18n.t('ui.extension.history.deleteHistory')} for ${session.task}`}
									>
