...
export function ConfigPanel({ config, onSave, onClose }: ConfigPanelProps) {
	...
	const i18n = useI18n(language || 'en-US')
	...
	return (
		<div className="flex flex-col gap-4 p-4 relative">
			<div className="flex items-center justify-between">
				<h2 className="text-base font-semibold">{i18n.t('ui.extension.config.title')}</h2>
				<Button
					variant="ghost"
					size="icon-sm"
					onClick={onClose}
			...
			<div className="flex flex-col gap-1.5 p-3 bg-muted/50 rounded-md border">
				<label htmlFor="user-auth-token" className="text-xs font-medium text-muted-foreground">
					{i18n.t('ui.extension.config.userAuthToken')}
				</label>
				<p className="text-[10px] text-muted-foreground mb-1">
					{i18n.t('ui.extension.config.userAuthTokenDescription')}
				</p>
			...
			{isTestingEndpoint(baseURL) && (
				<div className="p-2.5 rounded-md border border-amber-500/30 bg-amber-500/5 text-[11px] text-muted-foreground leading-relaxed">
					...
					{i18n.t('ui.extension.config.testingApiNotice')}
					<a
			...
			<div className="flex flex-col gap-1.5">
				<label htmlFor="model" className="text-xs text-muted-foreground">
					{i18n.t('ui.extension.config.model')}
				</label>
				<Input
					id="model"
					placeholder="gpt-5.1"
			...
			<div className="flex flex-col gap-1.5">
				<label htmlFor="api-key" className="text-xs text-muted-foreground">
					{i18n.t('ui.extension.config.apiKey')}
				</label>
				<div className="flex gap-2 items-center">
					<Input
						id="api-key"
			...
			<div className="flex flex-col gap-1.5">
				<label className="text-xs text-muted-foreground">
					{i18n.t('ui.extension.config.responseLanguage')}
				</label>
				<select
					value={language ?? ''}
				...
				>
					<option value="">{i18n.t('ui.extension.config.system')}</option>
					<option value="en-US">{i18n.t('ui.extension.config.english')}</option>
					<option value="zh-CN">{i18n.t('ui.extension.config.chinese')}</option>
				</select>
			</div>
			...
			>
				{i18n.t('ui.extension.config.advanced')}
				...
				<>
					<div className="flex flex-col gap-1.5">
						<label htmlFor="max-steps" className="text-xs text-muted-foreground">
							{i18n.t('ui.extension.config.maxSteps')}
						</label>
						<Input
							id="max-steps"
							type="number"
					...
					<div className="flex flex-col gap-1.5">
						<label className="text-xs text-muted-foreground">
							{i18n.t('ui.extension.config.systemInstruction')}
						</label>
						<textarea
					...
					<label className="flex items-center justify-between cursor-pointer">
						<span className="text-xs text-muted-foreground">
							{i18n.t('ui.extension.config.disableNamedToolChoice')}
						</span>
					...
					<label className="flex items-center justify-between cursor-pointer">
						<span className="text-xs text-muted-foreground">
							{i18n.t('ui.extension.config.experimentalLlmsTxt')}
						</span>
					...
					<label className="flex items-center justify-between cursor-pointer">
						<span className="text-xs text-muted-foreground">
							{i18n.t('ui.extension.config.experimentalIncludeAllTabs')}
						</span>
						<Switch
			...
			<div className="flex gap-2 mt-2">
				<Button variant="outline" onClick={onClose} className="flex-1 h-8 text-xs cursor-pointer">
					{i18n.t('ui.extension.config.cancel')}
				</Button>
				<Button
					onClick={handleSave}
					disabled={saving}
				...
				>
					{saving ? (
						...
						i18n.t('ui.extension.config.save')
					)}
				</Button>
			</div>
			...
			<div className="mt-4 mb-4 pt-4 border-t border-border/50 flex gap-2 justify-between text-[10px] text-muted-foreground">
				<div className="flex flex-col justify-between">
					<span>
						{i18n.t('ui.extension.config.version')}{' '}
					...
					>
						...
						<span>{i18n.t('ui.extension.config.sourceCode')}</span>
					</a>
				</div>
				...
				<div className="flex flex-col items-end">
					...
					>
						...
						<span>{i18n.t('ui.extension.config.homePage')}</span>
					</a>
					...
					>
						...
						<span>{i18n.t('ui.extension.config.privacy')}</span>
					</a>
				</div>
			</div>
			...
			<div className="text-[10px] text-muted-foreground bg-background fixed bottom-0 w-full flex justify-around">
				<span className="leading-loose">
					{i18n.t('ui.extension.config.builtWith')}{' '}
					<a
